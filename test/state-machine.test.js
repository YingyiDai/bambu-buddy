...
// ── filamentColor：打印报文带 AMS 颜色时随状态输出，供渲染层给「吃耗材」动画改色 ──
test('filamentColor：RUNNING 报文带 ams tray_color 时输出 #rrggbb', () => {
  const r = resolveState({
    connected: true, gcode_state: 'RUNNING', stg_cur: 0, mc_percent: 30,
    ams: { tray_now: '0', ams: [{ id: '0', tray: [{ id: '0', tray_color: 'F95959FF' }] }] },
  });
  assert.equal(r.stateKey, 'printing_25');
  assert.equal(r.filamentColor, '#f95959');
});

test('filamentColor：无 ams 信息时为 null（渲染层保持原始绿）', () => {
  const r = resolveState({ connected: true, gcode_state: 'RUNNING', stg_cur: 0, mc_percent: 30 });
  assert.equal(r.filamentColor, null);
});

test('filamentColor：外挂料盘（tray_now=254）读 vt_tray', () => {
  const r = resolveState({
    connected: true, gcode_state: 'RUNNING', stg_cur: 0, mc_percent: 80,
    ams: { tray_now: '254' }, vt_tray: { tray_color: '00AE42FF' },
  });
  assert.equal(r.filamentColor, '#00ae42');
});
