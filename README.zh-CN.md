...
打完了、还是出错了。瞄一眼就知道，不用切换到 Bambu Studio 或 Bambu Handy。

<div align="center"><img src="assets/readme/whatsthis.png" width="720" alt="Bambu Buddy 桌面实拍" /></div>
...
- 🖨️ **云端与局域网** —— 支持 Bambu 账号登录（海外区含 Google / Apple / Facebook 第三方账号，登录在官方页面完成、密码不经过本应用）或填 IP + access code 直连本地打印机。
- 🔀 **多打印机** —— 云端 + 本地打印机合并成一个列表，托盘菜单一键切换。
- 🚀 **开机自启动** —— 可在托盘菜单里开启「开机时自动启动」，登录系统后熊猫自动上桌面；默认关闭，不想要的用户不受打扰。
...
| 方式 | 怎么用 |
|---|---|
| 🎮 **探索** | 不用打印机，在「探索」里点一下就能看任意状态，或自动轮播。 |
| ☁️ **云端** | 登录 Bambu 账号（海外区支持官方页面登录，含三方账号），自动同步云端打印机，MQTT 订阅实时状态。 |
| 🏠 **局域网** | 填打印机 IP + access code（在打印机屏幕上查），直连本地打印机。 |

<div align="center"><img src="assets/readme/connectprinter.png" width="720" alt="连接打印机" /></div>
...
**它会上传我的数据吗？**
不会。账号凭据用系统钥匙串本地加密（macOS Keychain / Windows DPAPI，经 Electron `safeStorage`），只用于连接你自己的打印机；不向任何第三方服务器发送数据。

**怎么更新？**
托盘菜单或「设置 › 关于」点「检查更新」，会比对 GitHub 最新 Release，有新版一键跳转下载。

**没有拓竹打印机能用吗？**
能。「探索」就是为此准备的——纯欣赏熊猫的各种状态动画。
