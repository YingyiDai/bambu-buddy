...
// 变宽立即生效（长标签要能完整显示）；变窄延迟落定（滞回）。为何滞回：探索模式自动播放
// 里剩余时间/进度快速跳变，标签宽度在某个台阶边界附近来回抖，若立即按变窄收窄窗口，会与
// 随后的变宽反复拉锯、熊猫抖动。延迟期间窗口保持在近期较宽值，等标签稳定一段时间再收窄一次。
const LABEL_SHRINK_DELAY_MS = 600;
